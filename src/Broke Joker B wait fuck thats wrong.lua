local isMultiplayer = SMODS.find_mod("Multiplayer")[1]
SMODS.Joker {
    key = "sampletext",
    ...
    add_to_deck = function(self, card, from_debuff)
        G.GAME.neonmod_domnoen = true
    end,
    remove_from_deck = function(self, card, from_debuff)
        G.GAME.neonmod_domnoen = false
    end,
    calculate = function(self, card, context)
        if context.joker_main and not context.blueprint then
            G.GAME.neonmod_domnoen = true -- just for the sake of getting it to work with being spawned
        end
    end,
...
--- Three hyphens in this file is me (Neon), two is from me stealing this code from mp_gradient bullshit
...
local function set_temp_id(card, key)
    ...
    if card.orig.id <= 14 and card.orig.id >= 2 then --- to prevent modded ranks from fucking this up
        card.base.id = (16 - card.orig_id)
        ...
        end]] --- probably not needed for this one lol
    end
end

-- if code runs twice it needs a func ig
local function reset_ids()
    if G.GAME.neonmod_domnoen then
        G.GAME.neonmod_domnoen = nil
    ...
    ]] --- That person's writing is this fire???
end
...
local function valid_trigger(card, joker)
    ...
    local function rank_check(ranks)
        ...
        return                     --- we returning nothing with this one???
    end
...
function Card:is_face(from_boss)
    ...
    if G.GAME.neonmod_domnoen and not G.GAME.modifiers.mp_gradient and not G.MP_GRADIENT then --- Because I don't even want to imagine the horrors of this on Gradient, thanks
        local id = self:get_id()                                                              -- like seriously i want an explanation
        if self.debuff and not from_boss then return end
        if not ret and id == 3 or id == 4 or id == 5 then return true end
    end
    return ret
end
...
function Card:calculate_joker(context)
    if not context.blueprint then -- very important because bloopy recursively calls this
        if G.GAME.modifiers.mirror_rim and (context.other_card or passkey(self)) and not blacklist(self) then
            --(don't think this for is neccesary for this) for i = 1, 3 do
            --G.MP_GRADIENT = -i + 2
...
function Card:update(dt)
    ...
    if G.GAME.modifiers.mirror_rim then
        if self.ability.name == "Cloud 9" then
            self.ability.nine_tally = 0
            for k, v in pairs(G.playing_cards) do
                local id = v:get_id()
                if id == 7 or id == 9 then self.ability.nine_tally = self.ability.nine_tally + 1 end
            end
        end
    end
end
